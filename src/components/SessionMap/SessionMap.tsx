  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (isPanning) {
        ...
        if (viewBoxX + dx > minX && viewBoxX + dx < maxX)
          setViewBoxX(viewBoxX + dx);
        if (viewBoxY + dy > minY && viewBoxY + dy < maxY)
          setViewBoxY(viewBoxY + dy);

        if (viewBoxX > maxX) setViewBoxX(maxX);
        if (viewBoxX < minX) setViewBoxX(minX);
        if (viewBoxY > maxY)
          setViewBoxY((SVG_WIDTH * SVG_HEIGHT_MULTIPLIER) / 4);
  ...
  return (
    <div className={props.className}>
      <div onMouseLeave={() => setHoveredElementCountryName('')}>
        ...
        >
          {WorldSVGData.filter(
            element => element.name !== hoveredElementCountryName
          ).map((item, key) =>
            SVGCountryPath(
              cn('stroke-white stroke-[0.5] hover:cursor-pointer select-none'),
              item,
              key
            )
          )}
          {WorldSVGData.filter(
            ...
            return SVGCountryPath(
              cn(
                'stroke-gray-700 stroke-[0.5] hover:cursor-pointer select-none'
              ),
              item,
              key
            );
        ...
        <div
          className={cn(
            'absolute flex flex-col rounded-md border-[1px] bg-white p-2 shadow-sm',
            {
        ...
        >
          <div className="flex flex-row">
            <span
              className={cn(
                'fi mr-2',
                `fi-${getCountryCode(hoveredElementCountryName).toLowerCase()}`
              )}
            />
